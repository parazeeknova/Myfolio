...
const ye = Object.defineProperties
...
function de (i) {
  const { swiper: e, targetPosition: t, side: s } = i
  const n = z()
  const r = -e.translate
  let l = null
  let o
  ...
  const c = (p, f) => (d === 'next' && p >= f) || (d === 'prev' && p <= f)
  const u = () => {
    (o = new Date().getTime()), l === null && (l = o)
    const p = Math.max(Math.min((o - l) / a, 1), 0)
    const f = 0.5 - Math.cos(p * Math.PI) / 2
    let m = r + f * (t - r)
    if (
      (c(m, t) && (m = t),
    ...
    !e ||
      ...
      ((r = new ResizeObserver((u) => {
        l = n.requestAnimationFrame(() => {
          ...
          let w = f
          u.forEach((T) => {
            const { contentBoxSize: h, contentRect: x, target: g } = T;
            (g && g !== e.el) ||
...
function je () {
  ...
  const a = i.virtual && t.virtual.enabled
  const d = a ? i.virtual.slides.length : i.slides.length
  const c = O(n, `.${i.params.slideClass}, swiper-slide`)
  const u = a ? i.virtual.slides.length : c.length
  let p = []
  const f = []
  const m = []
  ...
  for (let S = 0; S < u; S += 1) {
    ...
    if (
      ...
      if (t.slidesPerView === 'auto') {
        ...
        const b = y.style.transform
        const C = y.style.webkitTransform
        if (
          ...
          const L = e(E, 'width')
          const k = e(E, 'padding-left')
          const Se = e(E, 'padding-right')
          const ie = e(E, 'margin-left')
          const se = e(E, 'margin-right')
          const re = E.getPropertyValue('box-sizing')
  ...
  if (
    ...
    const y = i.el.classList.contains(S)
...
function Xe (i) {
  ...
  for (let a = 0; a < s.length; a += 1) {
    ...
    const p =
      ...
      (d.swiperSlideSize + o)
    const f = -(l - c)
    const m = f + e.slidesSizesGrid[a]
    const w = f >= 0 && f <= e.size - e.slidesSizesGrid[a];
    ((f >= 0 && f < e.size - 1) ||
      (m > 1 && m <= e.size) ||
      (f <= 0 && m >= e.size)) &&
...
function Ke (i) {
  ...
  if (s === 0) (n = 0), (r = !0), (l = !0)
    ...
    const u = Math.abs(i - e.maxTranslate()) < 1;
    (r = c || n <= 0), (l = u || n >= 1), c && (n = 0), u && (n = 1)
  }
  if (t.loop) {
    const c = e.getSlideIndexByData(0)
    const u = e.getSlideIndexByData(e.slides.length - 1)
    const p = e.slidesGrid[c]
    const f = e.slidesGrid[u]
...
function Ue () {
  const i = this
  const { slides: e, params: t, slidesEl: s, activeIndex: n } = i
  const r = i.virtual && t.virtual.enabled
  const l = i.grid && t.grid && t.grid.rows > 1
  const o = (u) => O(s, `.${t.slideClass}${u}, swiper-slide${u}`)[0]
  e.forEach((u) => {
...
function Je (i) {
  ...
  const s = i.rtlTranslate ? i.translate : -i.translate
  let n
...
function tt (i) {
  ...
  const e = this
  const { params: t, rtlTranslate: s, translate: n, wrapperEl: r } = e
...
function it (i, e) {
  const t = this
  const { rtlTranslate: s, params: n, wrapperEl: r, progress: l } = t
  let o = 0
  let a = 0
  const d = 0
...
function ft (i, e, t, s, n) {
  ...
  if (o.normalizeSlideIndex) {
    for (let g = 0; g < d.length; g += 1) {
      ...
      const P = Math.floor(d[g] * 100)
      const M = Math.floor(d[g + 1] * 100)
...
function St (i) {
  ...
  const r = e.grid && t.grid && t.grid.rows > 1
  const l = t.slidesPerGroup * (r ? t.grid.rows : 1)
  const o = e.slides.length % l != 0
  const a = r && e.slides.length % t.grid.rows != 0
  const d = (c) => {
    for (let u = 0; u < c; u += 1) {
      const p = e.isElement
...
function Ct (i) {
  ...
  const { params: n, touches: r, rtlTranslate: l, enabled: o } = t
...
function Lt (i) {
  ...
  l.rewind &&
    (e.isBeginning
      ...
      : e.isEnd && (T = 0))
  const x = (f - d[m]) / w
  const g = m < l.slidesPerGroupSkip - 1 ? 1 : l.slidesPerGroup
  if (p > l.longSwipesMs) {
    if (!l.longSwipes) {
...
function he () {
  ...
  const l = i.virtual && i.params.virtual.enabled;
  (i.allowSlideNext = !0),
  (i.allowSlidePrev = !0),
  i.updateSize(),
  i.updateSlides(),
...
function At () {
  const i = this
  const { wrapperEl: e, rtlTranslate: t, enabled: s } = i
  if (!s) return;
...
const ge = (i, e) => {
  const t = B()
  const { params: s, el: n, wrapperEl: r, device: l } = i
  const o = !!s.nested
...
function Ft () {
  const i = this
  const { realIndex: e, initialized: t, params: s, el: n } = i
...
function Bt (i, e, t) {
  ...
  let s = !1
  const n = z()
  const r = e === 'window' ? n.innerHeight : t.clientHeight
  const l = Object.keys(i).map((o) => {
...
function _t () {
  const i = this
  const { classNames: e, params: t, rtl: s, el: n, device: r } = i
  const l = $t(
    [
      'initialized',
      t.direction,
...
function Wt () {
  const i = this
  const { isLocked: e, params: t } = i
  const { slidesOffsetBefore: s } = t
  if (s) {
    const n = i.slides.length - 1
    const r = i.slidesGrid[n] + i.slidesSizesGrid[n] + s * 2
    i.isLocked = i.size > r
...
class G {
  ...
  getSlideIndex (e) {
    ...
    const n = O(t, `.${s.slideClass}, swiper-slide`)
    const r = ce(n[0])
    return ce(e) - r
  }
...
G.use([_e, Re])
function Xt (i) {
  const { swiper: e, extendParams: t, on: s } = i
  t({
    parallax: {
      enabled: !1
    }
  ...
  const r = (a, d) => {
    const { rtl: c } = e
    const u = c ? -1 : 1
    const p = a.getAttribute('data-swiper-parallax') || '0'
    let f = a.getAttribute('data-swiper-parallax-x')
    let m = a.getAttribute('data-swiper-parallax-y')
    const w = a.getAttribute('data-swiper-parallax-scale')
    const T = a.getAttribute('data-swiper-parallax-opacity')
    const h = a.getAttribute('data-swiper-parallax-rotate')
    if (
      (f || m
        ? ((f = f || '0'), (m = m || '0'))
        : e.isHorizontal()

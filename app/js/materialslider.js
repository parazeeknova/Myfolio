...
function me (t) {
  const { swiper: e, targetPosition: i, side: s } = t
  const n = k()
  const r = -e.translate
  let l = null
  let o
  ...
  const c = (p, u) => (d === 'next' && p >= u) || (d === 'prev' && p <= u)
  const f = () => {
    (o = new Date().getTime()), l === null && (l = o)
    const p = Math.max(Math.min((o - l) / a, 1), 0)
    const u = 0.5 - Math.cos(p * Math.PI) / 2
    let m = r + u * (i - r)
    if (
      (c(m, i) && (m = i),
...
let K
function Oe (t) {
  const { userAgent: e } = t === void 0 ? {} : t
  const i = he()
  const s = k()
  const n = s.navigator.platform
  const r = e || s.navigator.userAgent
  const l = {
    ios: !1,
    android: !1
  }
  const o = s.screen.width
  const a = s.screen.height
  const d = r.match(/(Android);?[\s\/]+([\d.]+)?/)
  let c = r.match(/(iPad).*OS\s([\d_]+)/)
  const f = r.match(/(iPod)(.*OS\s([\d_]+))?/)
  const p = !c && r.match(/(iPhone\sOS|iOS)\s([\d_]+)/)
  const u = n === 'Win32'
  let m = n === 'MacIntel'
  const v = [
    '1024x1366',
    ...
    !e ||
      ...
      ((r = new ResizeObserver((f) => {
        l = n.requestAnimationFrame(() => {
          ...
          let v = u
          f.forEach((y) => {
            const { contentBoxSize: g, contentRect: E, target: S } = y;
            (S && S !== e.el) ||
...
function De (t) {
  const { swiper: e, extendParams: i, on: s, emit: n } = t
  const r = []
  const l = k()
...
function Fe () {
  ...
  const a = t.virtual && i.virtual.enabled
  const d = a ? t.virtual.slides.length : t.slides.length
  const c = V(n, `.${t.params.slideClass}, swiper-slide`)
  const f = a ? t.virtual.slides.length : c.length
  let p = []
  const u = []
  const m = []
  ...
  for (let w = 0; w < f; w += 1) {
    ...
    if (
      ...
      if (i.slidesPerView === 'auto') {
        ...
        const b = T.style.transform
        const z = T.style.webkitTransform
        if (
          ...
          const L = e(P, 'width')
          const O = e(P, 'padding-left')
          const F = e(P, 'padding-right')
          const B = e(P, 'margin-left')
          const re = e(P, 'margin-right')
          const ne = P.getPropertyValue('box-sizing')
  ...
  if (
    ...
    const T = t.el.classList.contains(w)
...
function Re (t) {
  ...
  const e = this
  const i = e.params
  const { slides: s, rtlTranslate: n, snapGrid: r } = e
  ...
  for (let a = 0; a < s.length; a += 1) {
    ...
    const p =
      ...
      (d.swiperSlideSize + o)
    const u = -(l - c)
    const m = u + e.slidesSizesGrid[a]
    const v = u >= 0 && u <= e.size - e.slidesSizesGrid[a];
    ((u >= 0 && u < e.size - 1) ||
      (m > 1 && m <= e.size) ||
      (u <= 0 && m >= e.size)) &&
...
function $e (t) {
  ...
  if (s === 0) (n = 0), (r = !0), (l = !0)
    ...
    const f = Math.abs(t - e.maxTranslate()) < 1;
    (r = c || n <= 0), (l = f || n >= 1), c && (n = 0), f && (n = 1)
  }
  if (i.loop) {
    const c = e.getSlideIndexByData(0)
    const f = e.getSlideIndexByData(e.slides.length - 1)
    const p = e.slidesGrid[c]
    const u = e.slidesGrid[f]
    const m = e.slidesGrid[e.slidesGrid.length - 1]
    const v = Math.abs(t)
...
function He () {
  ...
  const r = t.virtual && i.virtual.enabled
  const l = t.grid && i.grid && i.grid.rows > 1
  const o = (f) => V(s, `.${i.slideClass}${f}, swiper-slide${f}`)[0]
  e.forEach((f) => {
...
function Xe (t) {
  ...
  const e = this
  const { params: i, rtlTranslate: s, translate: n, wrapperEl: r } = e
...
function Ke (t, e) {
  const i = this
  const { rtlTranslate: s, params: n, wrapperEl: r, progress: l } = i
  let o = 0
  let a = 0
  const d = 0
...
function ge (t) {
  const { swiper: e, runCallbacks: i, direction: s, step: n } = t
...
function rt (t, e, i, s, n) {
  ...
  if (o.normalizeSlideIndex) {
    for (let S = 0; S < d.length; S += 1) {
      ...
      const x = Math.floor(d[S] * 100)
      const I = Math.floor(d[S + 1] * 100)
      typeof d[S + 1] < 'u'
        ? h >= x && h < I - (I - x) / 2
          ? (l = S)
          : h >= x && h < I && (l = S + 1)
  ...
  if (n.params.loop) {
    if (n.virtual && n.params.virtual.enabled) l = l + n.virtual.slidesBefore
      ...
      const a = r
        ...
        : n.slides.length
      const { centeredSlides: d } = n.params
...
function at (t, e, i) {
  ...
  const s = this
  const { enabled: n, params: r, animating: l } = s
  if (!n) return s
...
function dt (t, e, i, s) {
  ...
  const o = l + Math.floor((r - l) / n.params.slidesPerGroup)
  const a = n.rtlTranslate ? n.translate : -n.translate
  if (a >= n.snapGrid[o]) {
...
function ut (t) {
  ...
  const r = e.grid && i.grid && i.grid.rows > 1
  const l = i.slidesPerGroup * (r ? i.grid.rows : 1)
  const o = e.slides.length % l !== 0
  const a = r && e.slides.length % i.grid.rows !== 0
  const d = (c) => {
    for (let f = 0; f < c; f += 1) {
      const p = e.isElement
...
function pt (t) {
  ...
  const P = (E ? d[r].column : r) + (m && typeof n > 'u' ? -v / 2 + 0.5 : 0)
  if (P < g) {
    C = Math.max(g - P, y)
...
function de (t, e, i) {
  const s = k()
  const { params: n } = t
  const r = n.edgeSwipeDetection
  const l = n.edgeSwipeThreshold
...
function xt (t) {
  ...
  const { params: n, touches: r, rtlTranslate: l, enabled: o } = i
...
function yt (t) {
  ...
  l.rewind &&
    (e.isBeginning
      ...
      : e.isEnd && (g = 0))
  const S = (u - d[v]) / y
  const h = v < l.slidesPerGroupSkip - 1 ? 1 : l.slidesPerGroup
  if (p > l.longSwipesMs) {
    if (!l.longSwipes) {
...
function ce () {
  const t = this
  const { params: e, el: i } = t
  if (i && i.offsetWidth === 0) return
  e.breakpoints && t.setBreakpoint()
  const { allowSlideNext: s, allowSlidePrev: n, snapGrid: r } = t
  const l = t.virtual && t.params.virtual.enabled;
  (t.allowSlideNext = !0),
  (t.allowSlidePrev = !0),
  t.updateSize(),
  t.updateSlides(),
...
function Et () {
  const t = this
  const { wrapperEl: e, rtlTranslate: i, enabled: s } = t
  if (!s) return;
...
const ve = (t, e) => {
  const i = N()
  const { params: s, el: n, wrapperEl: r, device: l } = t
  const o = !!s.nested
  const a = e === 'on' ? 'addEventListener' : 'removeEventListener'
  const d = e
...
function Ot () {
  const t = this
  const { realIndex: e, initialized: i, params: s, el: n } = t
...
function zt (t, e, i) {
  ...
  let s = !1
  const n = k()
  const r = e === 'window' ? n.innerHeight : i.clientHeight
  const l = Object.keys(t).map((o) => {
...
function kt () {
  const t = this
  const { classNames: e, params: i, rtl: s, el: n, device: r } = t
  const l = Gt(
    [
      'initialized',
      i.direction,
...
function _t () {
  const t = this
  const { isLocked: e, params: i } = t
  const { slidesOffsetBefore: s } = i
  if (s) {
    const n = t.slides.length - 1
    const r = t.slidesGrid[n] + t.slidesSizesGrid[n] + s * 2
    t.isLocked = t.size > r
...
class D {
  ...
  getSlideIndex (e) {
    ...
    const n = V(i, `.${s.slideClass}, swiper-slide`)
    const r = le(n[0])
    return le(e) - r
  }
  ...
  update () {
    ...
    function n () {
      ...
      const o = Math.min(Math.max(l, e.maxTranslate()), e.minTranslate())
  ...
  destroy (e, i) {
    ...
    const s = this
    const { params: n, el: r, wrapperEl: l, slides: o } = s
    return (
      typeof s.params > 'u' ||
        s.destroyed ||
...
function $t ({ swiper: t, on: e, extendParams: i }) {
  ...
  const s = () => {
    ...
    for (let v = 0; v < r.length; v += 1) {
      const y = r[v]
      const g = y.querySelector('.swiper-material-wrapper')
      const E = y.querySelectorAll('.swiper-material-animate-opacity')
      const S = y.querySelectorAll('[data-swiper-material-scale]')
      const h = -y.progress
      const x = y.swiperSlideOffset
      const I = t.translate
      let M
      let C = 0
      let A = 0
      const w = l[v]
      const T = m === 0 && !p ? 0 : c / 2 / w
      const P = f ? I : 0
      const b = p && d % 2 === 1
      const z = p && d % 2 === 0
      if (h <= 0) {
  ...
  const n = (r) => {
    ...
    for (let a = 0; a < l.length; a += 1) {
      const d = l[a]
      const c = d.querySelector('.swiper-material-wrapper')
      const f = d.querySelectorAll('.swiper-material-animate-opacity')
      const p = d.querySelectorAll('[data-swiper-material-scale]');
      [c, ...p, ...f].forEach((u) => {
...
const Ht = document.querySelector('input')
